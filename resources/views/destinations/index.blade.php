...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Search Bar (untuk halaman explore) -->
        <div class="mb-6">
        ...
        <div class="mb-6 overflow-x-auto pb-2">
            <div class="flex space-x-2">
                <a href="{{ route('destinations.index', array_merge(request()->query(), ['category' => null])) }}" 
                   class="px-4 py-2 rounded-full {{ !request('category') ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}">
                    Semua
                </a>
                @foreach($categories as $category)
                    <a href="{{ route('destinations.index', array_merge(request()->query(), ['category' => $category->slug])) }}" 
                       class="px-4 py-2 rounded-full whitespace-nowrap {{ request('category') == $category->slug ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}">
                        @if($category->icon)
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex items-center space-x-2">
                ...
                <select onchange="window.location.href = this.value" class="border border-gray-300 rounded-lg px-3 py-1">
                    <option value="{{ route('destinations.index', array_merge(request()->except('sort'), ['sort' => 'az'])) }}" 
                        {{ request('sort') == 'az' || !request('sort') ? 'selected' : '' }}>
                        A-Z
                    </option>
                    <option value="{{ route('destinations.index', array_merge(request()->except('sort'), ['sort' => 'za'])) }}" 
                        {{ request('sort') == 'za' ? 'selected' : '' }}>
                        Z-A
                    </option>
                    <option value="{{ route('destinations.index', array_merge(request()->except('sort'), ['sort' => 'rating'])) }}" 
                        {{ request('sort') == 'rating' ? 'selected' : '' }}>
                        Rating Tertinggi
                    </option>
                    <option value="{{ route('destinations.index', array_merge(request()->except('sort'), ['sort' => 'popular'])) }}" 
                        {{ request('sort') == 'popular' ? 'selected' : '' }}>
                        Terpopuler
                    </option>
                    <option value="{{ route('destinations.index', array_merge(request()->except('sort'), ['sort' => 'newest'])) }}" 
